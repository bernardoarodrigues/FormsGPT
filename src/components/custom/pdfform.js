...
function visit_acroform_fields(doc, callback) {
	if (doc.acroForm) {
		...
		while (to_visit.length > 0) {
			...
			// BERNARDO: updated below to get options from radio buttons
			// if (n.map && n.map.Kids && n.map.Opt && n.map.FT && (n.map.FT.name === 'Btn')) {
			if (n.map && n.map.Kids && n.map.FT && (n.map.FT.name === 'Btn')) {
				// Radio button group
				var options = [];
				n.map.Kids.forEach(function(kid_ref) {
					var kid = doc.fetch(kid_ref);

					if(kid.map.AP) {
						Object.keys(kid.map.AP.map.N.map).forEach(function(key) {
							if (key !== 'Off' && key !== 'On') {
								options.push(key);
							}
						});
...
function transform(buf, fields) {
	...
	visit_acroform_fields(doc, function(n) {
		...
		// Bernardo: Updated "Yes" to value in line 418
		if (n._pdfform_spec) {
			...
			if (type === 'radio') {
				...
				kid.map.AS = kid.map.V = kid.map.DV = new minipdf_lib.Name("1"); 
			...
			// Bernardo: Updated "Yes" to "On" in line 432
